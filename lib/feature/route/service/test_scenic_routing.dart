...
class ScenicRoutingTester {
  ...
  static Future<void> testBalancedMode() async {
    _logger.i('=' * 80);
    _logger.i('TEST 2: BALANCED MODE ');
    _logger.i('=' * 80);
    final service = SmartRoutingService();
    await service.initialize();
    final start = LatLng(25.2048, 55.2708);
  ...
  static Future<void> testExplorationMode() async {
    _logger.i('=' * 80);
    _logger.i('TEST 4: EXPLORATION MODE ');
    _logger.i('=' * 80);
    final service = SmartRoutingService();
    await service.initialize();
    final start = LatLng(25.2048, 55.2708);
  ...
  static void _printRouteResults(dynamic route, String modeName) {
    ...
    if (route.points.length > 2) {
      ...
      _logger.w('  WARNING: Route only has ${route.points.length} points');
    }
  ...
  static void _analyzeSpiralPattern(dynamic route, LatLng start, LatLng end) {
    ...
    if (maxDistance - minDistance > 500) {
      ...
      _logger.w('     Low variation - route may not be very spiral-like');
    }
    var angleChanges = 0;
    if (route.points.length > 3) {
      ...
      if (angleChanges > route.points.length ~/ 4) {
        ...
        _logger.w('     Low angular variation - path may be too direct');
      }
    }
    _logger.i('');
    ...
    if (spiralScore > 70) {
      ...
      _logger.i('     FAIR: Some winding, but not strongly spiral');
    } else {
      _logger.w('     POOR: Route is too direct, not spiral-like');
    }
  }
