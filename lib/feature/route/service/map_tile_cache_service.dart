...
/// Service for downloading and caching map tiles for offline use
class MapTileCacheService {
  ...
  TileCoordinate _latLngToTile(LatLng latLng, int zoom) {
    ...
    final y = ((1.0 - (latLng.latitude * pi / 180.0).tan().asinh() / pi) / 2.0 * n).floor();
    return TileCoordinate(z: zoom, x: x, y: y);
  }
  
  int _kmToTiles(double km, int zoom) {
    // Approximate: 1 tile at zoom level = 40075 km / 2^zoom at equator
